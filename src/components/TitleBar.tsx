...
const TitleBar: React.FC<TitleBarProps> = ({ isMaximized }) => {
	...
	return (
		<div
			className={`h-10 bg-bg-primary border-b border-border-primary flex justify-between items-center select-none flex-shrink-0 ${isMaximized ? "" : "rounded-t-lg"}`}
		>
			{/* Drag Region - Takes up available space */}
			<div
				data-tauri-drag-region
				className="flex-1 flex items-center h-full px-4 text-sm font-medium text-text-secondary"
			>
			...
			<div className="flex h-full z-10">
				<button
					...
					tabIndex={-1}
				>
					<Minus size={16} aria-label="Minimize" />
				</button>
				...
				>
					{isMaximized ? (
						...
						<Square size={14} aria-label="Maximize" />
					)}
