	...
	return (
		<div className="px-4 py-2 shadow-md rounded-md bg-bg-secondary border-2 border-border-secondary text-text-primary hover:border-accent-secondary transition-colors">
			...
			<Handle
				...
				id="top"
				className="w-16 !bg-accent-secondary"
			/>
			<Handle
				type="source"
				position={Position.Top}
		...
		<div className="h-full flex flex-col w-full">
			...
			<div className="flex-1 bg-bg-primary" ref={wrapperRef}>
				...
				{isEditOpen && (
					...
					>
						<div
							className="bg-bg-secondary border border-border-primary p-4 rounded shadow-lg w-64"
							onClick={(e) => e.stopPropagation()}
						>
							<h3 className="font-bold mb-2 text-sm text-text-primary">
								Edit Node
							</h3>
							<input
								type="text"
							...
							<div className="flex justify-end gap-2">
								...
								<button
									...
									className="px-2 py-1 text-xs bg-accent-secondary text-white rounded hover:bg-accent-hover"
								>
									Save
								</button>
							</div>
...
const PanelToAddNode = ({ setNodes }: { setNodes: any }) => {
	...
	return (
		<Panel position="top-right">
			<button
				onClick={addNode}
				className="bg-bg-secondary text-text-primary p-2 rounded shadow border border-border-primary text-sm hover:bg-bg-hover"
			>
				+ Add Node
			</button>
		</Panel>
